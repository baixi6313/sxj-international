...
// 真实网络公开事件：evt_001 南京博物院、evt_002 小红书期权、evt_004 耿同学学术打假、evt_006 Kimi分享异常、evt_008 百度搭子分享失败
...
App({
  ...
  globalData: {
    ...
    provenanceNote: '站内所记事现中，「南京博物院藏品流向」「耿同学学术打假」「小红书前员工期权」三件为网络公开、可独立核实的真实社会事件；「Kimi 分享链接异常」「百度搭子分享失败」为记录人亲历并附截图的平台功能异常记录；其余所有事现（含「红旗驿站」等）均为白玺与 AI 共同演绎生成（附时间戳可验证），仅供框架推演，非既成事实。'
  }
});
